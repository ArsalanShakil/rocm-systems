...
bool HasOneSourceAgent(const std::vector<const hsa_amd_memory_copy_op_t*>& operations) {
  const uint64_t src_agent_handle = operations.front()->src_agent.handle;
  for (const hsa_amd_memory_copy_op_t* operation : operations) {
...
bool IsIndirectCopy(uint16_t operation_type) {
  return operation_type == HSA_AMD_MEMORY_COPY_OP_LINEAR_INDIRECT_SRC ||
         operation_type == HSA_AMD_MEMORY_COPY_OP_LINEAR_INDIRECT_DST ||
         operation_type == HSA_AMD_MEMORY_COPY_OP_LINEAR_INDIRECT_SRCDST;
}
...
void PushLinearOperation(hsa_agent_t src_agent, CopyListStorage pending,
  ...
  CopyListStorage& stored = copy_list_storage.back();

  operation.src_list = stored.srcs.data();
  operation.dst_list = stored.dsts.data();
  operation.dst_agent_list = stored.dst_agents.data();
  operation.size_list = stored.sizes.data();
  operation.num_entries = static_cast<uint16_t>(stored.srcs.size());
  final_operations.push_back(operation);
}
...
void PushSwapOperations(const std::vector<const hsa_amd_memory_copy_op_t*>& swap_operations,
  ...
  swap.src_agent = swap_operations.front()->src_agent;
...
void PushLinearOperations(const std::vector<const hsa_amd_memory_copy_op_t*>& linear_operations,
  ...
  if (engine != HwQueueEngine::SdmaD2D) {
    if (engine == HwQueueEngine::SdmaH2D || HasOneSourceAgent(linear_operations)) {
      CopyListStorage pending;
      for (const hsa_amd_memory_copy_op_t* operation : linear_operations) {
        AddCopyListEntry(*operation, operation->size, pending);
      }
      PushLinearOperation(linear_operations.front()->src_agent, std::move(pending),
                          final_operations, copy_list_storage);
      return;
    }

    std::map<uint64_t, hsa_agent_t> src_agents;
    std::map<uint64_t, CopyListStorage> pending_by_src_agent;
    for (const hsa_amd_memory_copy_op_t* operation : linear_operations) {
      src_agents[operation->src_agent.handle] = operation->src_agent;
    ...
    for (auto& pending_entry : pending_by_src_agent) {
      PushLinearOperation(src_agents[pending_entry.first], std::move(pending_entry.second),
                          final_operations, copy_list_storage);
    }
    return;
  }
  ...
  for (const hsa_amd_memory_copy_op_t* operation : linear_operations) {
    ...
    src_agents[src_agent_handle] = operation->src_agent;
  ...
  for (auto& source_group : broadcast_groups) {
    ...
    for (auto& broadcast_group : source_group.second) {
      ...
      if (broadcast_entry.dsts.size() > 1) {
        ...
        broadcast.dst_list = stored.dsts.data();
        broadcast.dst_agent_list = stored.dst_agents.data();
        broadcast.num_entries = static_cast<uint16_t>(stored.dsts.size());
    ...
    PushLinearOperation(src_agents[source_group.first], std::move(pending), final_operations,
                        copy_list_storage);
  }
}
...
void PushIndirectOperations(const std::vector<const hsa_amd_memory_copy_op_t*>& indirect_operations,
                            std::vector<hsa_amd_memory_copy_op_t>& final_operations,
                            std::vector<CopyListStorage>& copy_list_storage) {
  std::map<hsa_amd_memory_copy_op_type_t, hsa_agent_t> src_agents;
  ...
  for (const hsa_amd_memory_copy_op_t* operation : indirect_operations) {
    ...
    if (pending_by_type[operation_type].srcs.empty()) {
      src_agents[operation_type] = operation->src_agent;
    }
...
void LogBatchCopyOperations(const std::vector<hsa_amd_memory_copy_op_t>& operations,
                            HwQueueEngine engine, const std::vector<hsa_signal_t>& wait_events) {
  for (size_t i = 0; i < operations.size(); ++i) {
    const auto& operation = operations[i];
    if (operation.type == HSA_AMD_MEMORY_COPY_OP_LINEAR_BROADCAST) {
      for (uint32_t d = 0; d < operation.num_entries; ++d) {
        ClPrint(amd::LOG_DEBUG, amd::LOG_COPY2,
                "HSA BatchCopy Broadcast [%u/%u] engineOp=%s, src=%p, dst=%p, "
                "size=%zu, wait_event=0x%zx, completion_signal=0x%zx",
                d + 1, operation.num_entries, EngineOpName(engine), operation.src,
                operation.dst_list[d], operation.size,
                (wait_events.size() != 0) ? wait_events[0].handle : 0,
                operation.completion_signal.handle);
      }
    } else if (operation.type == HSA_AMD_MEMORY_COPY_OP_LINEAR_SWAP) {
      for (uint32_t d = 0; d < operation.num_entries; ++d) {
        ClPrint(amd::LOG_DEBUG, amd::LOG_COPY2,
                "HSA BatchCopy Swap [%u/%u] engineOp=%s, addr_a=%p, addr_b=%p, "
                "size=%zu, wait_event=0x%zx, completion_signal=0x%zx",
                d + 1, operation.num_entries, EngineOpName(engine), operation.src_list[d],
                operation.dst_list[d], operation.size_list[d],
                (wait_events.size() != 0) ? wait_events[0].handle : 0,
                operation.completion_signal.handle);
      }
    } else if (IsIndirectCopy(operation.type)) {
      const char* indirect_kind =
          (operation.type == HSA_AMD_MEMORY_COPY_OP_LINEAR_INDIRECT_SRC)   ? "Src"
          : (operation.type == HSA_AMD_MEMORY_COPY_OP_LINEAR_INDIRECT_DST) ? "Dst"
                                                                           : "SrcDst";
      for (uint32_t d = 0; d < operation.num_entries; ++d) {
        ClPrint(amd::LOG_DEBUG, amd::LOG_COPY2,
                "HSA BatchCopy Indirect%s [%u/%u] engineOp=%s, dst=%p, src=%p, "
                "size=%zu, wait_event=0x%zx, completion_signal=0x%zx",
                indirect_kind, d + 1, operation.num_entries, EngineOpName(engine),
                operation.dst_list[d], operation.src_list[d], operation.size_list[d],
                (wait_events.size() != 0) ? wait_events[0].handle : 0,
                operation.completion_signal.handle);
      }
    } else if (operation.type == HSA_AMD_MEMORY_COPY_OP_LINEAR && operation.num_entries > 0) {
      for (uint32_t d = 0; d < operation.num_entries; ++d) {
        ClPrint(amd::LOG_DEBUG, amd::LOG_COPY2,
      ...
      ClPrint(amd::LOG_DEBUG, amd::LOG_COPY2,
              ...
              i, operations.size(), EngineOpName(engine), operation.dst, operation.src,
              operation.size, operation.src_agent.handle, operation.dst_agent.handle,
              (wait_events.size() != 0) ? wait_events[0].handle : 0,
              operation.completion_signal.handle);
    }
  }
...
bool DmaBlitManager::rocrCopyBufferBatch(const std::vector<hsa_amd_memory_copy_op_t>& copyOps,
  ...
  for (uint32_t e = 0; e < kNumCopyOpBins; ++e) {
    ...
    for (const auto& operation : ops) {
      ...
      if (IsIndirectCopy(operation.type)) {
    ...
    for (auto& operation : final_operations) {
      operation.completion_signal = gpu().Barriers().ActiveSignal(1, gpu().timestamp());
      groupSignals.push_back(gpu().Barriers().GetLastSignal());
    }

    if (IsLogEnabled(amd::LOG_DEBUG, amd::LOG_COPY2)) {
      LogBatchCopyOperations(final_operations, engine, wait_events);
    }
