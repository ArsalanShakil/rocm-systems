...
void Device::RevokeAccess(void* ptr, size_t size) const {
  if (ptr == nullptr) {
    return;
  }
  hsa_amd_svm_attribute_pair_t attr{HSA_AMD_SVM_ATTRIB_AGENT_NO_ACCESS,
                                    getBackendDevice().handle};
  hsa_status_t status = Hsa::svm_attributes_set(ptr, size, &attr, 1);
  if (status != HSA_STATUS_SUCCESS) {
    ClPrint(amd::LOG_DEBUG, amd::LOG_MEM,
            "RevokeAccess: NO_ACCESS revoke failed for %p, status: %d", ptr, status);
  }
}
