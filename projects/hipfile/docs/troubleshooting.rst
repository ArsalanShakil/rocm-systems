...
hipFile does not support NVMe multipath devices for fastpath IO.
...
used to determine offset alignment and IO size requirements
(``stx_dio_offset_align``), and memory alignment requirements
(``stx_dio_mem_align``) for the storage.

If an IO request does not meet alignment and size requirements, hipFile will use
the fallback path. ``ais-stats`` can be used to determine if IO requests are
...
For usage details and examples, see `Stats Collection Tool`_.
