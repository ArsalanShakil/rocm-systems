...
def create(root: Path) -> Path:
    ...
    Call with e.g. root = Path(tmp_path) / "fake-sysfs".
    """
    root = root.resolve()
    ...
    # Port PCI device (nested under bridge in the devices tree so that
    # downstream_port() finds "/" + BRIDGE_BDF + "/" in the canonical path)
    ...
    # IB port + hw_counters (the simulator writes counter values here)
    ...
    # Auxiliary device entry so IONIC_RDMA driver_loaded() returns true.
    # Its canonical path must contain "/" + PORT_BDF + "/".
    ...
    _write(net_iface_dir / "type",      "32")   # ARPHRD_INFINIBAND
    ...
    # class/net/<iface>/device  →  ../../../devices/pci0000:e0/<bridge>/<port>
    # Parent of this symlink is sys/class/net/<iface>/ — three levels below sys/.
    _symlink(
    ...
    # ------------------------------------------------------------------
    # /sys/class/infiniband/<ib_dev>  →  back to the real ib_dev_dir
    # ------------------------------------------------------------------
    ...
    # infiniband/<ib_dev>/subsystem  →  /sys/class/infiniband
    # (needed for the amdsmi_get_nic_rdma_port_statistics path traversal)
    _symlink(
    ...
    # ------------------------------------------------------------------
    # /sys/bus/pci/devices/ symlinks
    # ------------------------------------------------------------------
    pci_dev_dir = root / "sys/bus/pci/devices"
    pci_dev_dir.mkdir(parents=True, exist_ok=True)
    _symlink(
        pci_dev_dir / BRIDGE_BDF,
        "../../../devices/pci0000:e0/" + BRIDGE_BDF,
    )
    _symlink(
        pci_dev_dir / PORT_BDF,
        "../../../devices/pci0000:e0/" + BRIDGE_BDF + "/" + PORT_BDF,
    )

    # ------------------------------------------------------------------
    # /sys/bus/pci/drivers/ionic/<port_bdf>  (IONIC driver check)
    # The entry filename must equal PORT_BDF.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # /sys/bus/auxiliary/drivers/ionic_rdma.rdma/<entry>  (IONIC_RDMA check)
    # The canonical path of the symlink target must contain "/" + PORT_BDF + "/".
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # /sys/devices/system/node/node0/cpulist  (NUMA affinity)
    # ------------------------------------------------------------------
