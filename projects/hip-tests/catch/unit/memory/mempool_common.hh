...
class streamMemAllocTest {
  ...
  // When true, methods use the thread-safe (deferred) check macros so they can
  // be safely executed from worker threads. Call HIP_CHECK_THREAD_FINALIZE() on
  // the main thread after the worker threads join.
 ...
 public:
  ...
  // Thread-safe validation: returns false on mismatch instead of using Catch2
  // macros, so it can be safely called from worker threads.
