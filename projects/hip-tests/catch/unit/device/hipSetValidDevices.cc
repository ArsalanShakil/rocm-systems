...
static void performOperations(bool threadSafe = false) {
  ...
  HIP_CHECK_OPT_THREAD(threadSafe, hipMemcpy(devMem, hostMem, N * sizeof(int), hipMemcpyHostToDevice));

  doubleKernel<<<1, N>>>(devMem, N);

  HIP_CHECK_OPT_THREAD(threadSafe, hipMemcpy(hostMem, devMem, N * sizeof(int), hipMemcpyDeviceToHost));
  for (int i = 0; i < N; i++) {
...
void launchFunction(int deviceId) {
  REQUIRE_THREAD(getCurrentDevice() == 0);

  int length = 1;
  int deviceArr[1] = {deviceId};
  HIP_CHECK_THREAD(hipSetValidDevices(deviceArr, length));

  REQUIRE_THREAD(getCurrentDevice() == deviceId);
