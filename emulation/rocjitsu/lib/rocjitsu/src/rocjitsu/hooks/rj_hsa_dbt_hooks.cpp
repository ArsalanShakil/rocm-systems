...
constexpr std::array<TargetInfo, 10> kTargets = {{
...
[[nodiscard]] bool contains_string(const uint8_t *bytes, size_t size, std::string_view needle) {
  if (bytes == nullptr || needle.empty())
    return false;
...
[[nodiscard]] rj_code_arch_t detect_arch_from_elf(const uint8_t *bytes, size_t size) {
  ...
  // IREE gfx1250 HSACOs may carry RDNA4-like ELF machine flags while the
  // AMDHSA target note still records the real guest ISA. Prefer that explicit
  // target triple before falling back to EF_AMDGPU_MACH.
  const bool has_gfx1250_triple = contains_string(bytes, size, "amdgcn-amd-amdhsa--gfx1250");
  const bool has_gfx1200_triple = contains_string(bytes, size, "amdgcn-amd-amdhsa--gfx1200");
  const bool has_gfx1201_triple = contains_string(bytes, size, "amdgcn-amd-amdhsa--gfx1201");
  const bool has_gfx950_triple = contains_string(bytes, size, "amdgcn-amd-amdhsa--gfx950");
  const bool has_gfx940_triple = contains_string(bytes, size, "amdgcn-amd-amdhsa--gfx940");
  const bool has_gfx941_triple = contains_string(bytes, size, "amdgcn-amd-amdhsa--gfx941");
  const bool has_gfx942_triple = contains_string(bytes, size, "amdgcn-amd-amdhsa--gfx942");
  log_message(kLogDebug,
              "detect ELF size=%zu mach=0x%x target_strings: gfx1250=%d gfx1200=%d gfx1201=%d "
              "gfx950=%d gfx940=%d gfx941=%d gfx942=%d",
              size, header.e_flags & EF_AMDGPU_MACH, static_cast<int>(has_gfx1250_triple),
              static_cast<int>(has_gfx1200_triple), static_cast<int>(has_gfx1201_triple),
              static_cast<int>(has_gfx950_triple), static_cast<int>(has_gfx940_triple),
              static_cast<int>(has_gfx941_triple), static_cast<int>(has_gfx942_triple));

  if (has_gfx1250_triple)
    return ROCJITSU_CODE_ARCH_GFX1250;
  if (has_gfx1200_triple || has_gfx1201_triple)
    return ROCJITSU_CODE_ARCH_RDNA4;
  if (has_gfx950_triple)
    return ROCJITSU_CODE_ARCH_CDNA4;
  if (has_gfx940_triple || has_gfx941_triple || has_gfx942_triple)
