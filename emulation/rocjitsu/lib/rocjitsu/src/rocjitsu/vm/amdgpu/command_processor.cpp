...
// The 32-WG limit matches the current 32-bit cluster multicast mask.
constexpr uint32_t kMaxClusterWorkgroups = 32;
...
CommandProcessor::cluster_lds_targets(uint32_t dispatch_id, uint32_t wg_id, uint32_t mcast_mask) {
  ...
  for (uint32_t rank = 0; rank < src.cluster_size && rank < 32; ++rank) {
