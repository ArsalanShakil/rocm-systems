...
constexpr uint32_t VGPRS_PER_WF = 512;
...
struct MfmaBenchFixture {
  ...
  void seed_f16(uint64_t seed, uint32_t s0_regs, uint32_t s1_regs, uint32_t dst_regs) {
    ...
    for (uint32_t r = 0; r < s0_regs; ++r)
      for (uint32_t lane = 0; lane < WF_SIZE; ++lane)
        cu->write_vgpr(vb + 10 + r, lane, rand_f16x2());
    for (uint32_t r = 0; r < s1_regs; ++r)
      for (uint32_t lane = 0; lane < WF_SIZE; ++lane)
        cu->write_vgpr(vb + 50 + r, lane, rand_f16x2());
  ...
  void seed_i8(uint64_t seed, uint32_t s0_regs, uint32_t s1_regs, uint32_t dst_regs) {
    ...
    for (uint32_t r = 0; r < s0_regs; ++r)
      for (uint32_t lane = 0; lane < WF_SIZE; ++lane)
        cu->write_vgpr(vb + 10 + r, lane, static_cast<uint32_t>(rng()));
    for (uint32_t r = 0; r < s1_regs; ++r)
      for (uint32_t lane = 0; lane < WF_SIZE; ++lane)
        cu->write_vgpr(vb + 50 + r, lane, static_cast<uint32_t>(rng()));
    for (uint32_t r = 0; r < dst_regs; ++r)
      for (uint32_t lane = 0; lane < WF_SIZE; ++lane)
        cu->write_vgpr(vb + 256 + r, lane, 0);
  }
...
TEST(MfmaBenchmark, F32_32x32x8_F16) {
  ...
  uint32_t dst = fx.vb + 256, s0 = fx.vb + 10, s1 = fx.vb + 50, s2 = fx.vb + 256;
  auto s = time_mfma(
      [&] {
